...
export class removeSRO1656457788192 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      "UPDATE unit_group_unit_type_unit_types SET unit_types_id = (SELECT id FROM unit_types WHERE name = 'studio') WHERE unit_type_id = (SELECT id FROM unit_types WHERE name = 'SRO')"
    )
